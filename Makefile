BUILD_CGO_ENABLED  := 0
SERVICE_NAME       := whatsapp-go
SERVICE_PORT       := 3000
IMAGE_NAME         := whatsapp-go
IMAGE_TAG          := latest
REBASE_URL         := "github.com/ichadhr/whatsapp-go"
...
rebase:
	rm -rf .git
	find . -type f -iname "*.go*" -exec sed -i '' -e "s%github.com/ichadhr/whatsapp-go%$(REBASE_URL)%g" {} \;
	git init
